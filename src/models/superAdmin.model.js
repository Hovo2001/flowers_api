...
class superAdminModel {
    ...
    static async getAllProducts() {
        try {
            ...
            for (let i in selectAdminDetails) {
    ...
    //         const delSuperAdmin = await pg('super_admin').del().where('table_names','=',dropColumns.tableName).returning('*');pg.destroy();
    ...
    //     try {
    //         const selectedData = await pg('boxparams').insert({ ...payload, created_at: new Date() });return 'success';
    //     } catch (error) {
    //         console.log(error);
    //         return 'failed';
    //     }
